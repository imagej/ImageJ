...
public class Toolbar extends Canvas implements MouseListener, MouseMotionListener, ItemListener, ActionListener {
	...
	*/
	...
	void showBrushDialog() {
		ij.gui.GenericDialog gd = new ij.gui.GenericDialog("Selection Brush");
		...
		ij.gui.Roi roi = img!=null?img.getRoi():null;
		if (roi!=null && roi.getType()== ij.gui.Roi.OVAL && ovalType==BRUSH_ROI)
			img.deleteRoi();
		Prefs.set(BRUSH_SIZE, brushSize);
	}

	void showAngleDialog() {
		ij.gui.GenericDialog gd = new ij.gui.GenericDialog("Angle Tool");
