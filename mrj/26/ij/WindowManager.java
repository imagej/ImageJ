...
public class WindowManager {
	...
	*/
	...
	public synchronized static String[] getImageTitles() {
		...
		for (int i=0; i<list.length; i++) {
			ij.ImagePlus img = getImage(list[i]);
	...
	/** For IDs less than zero, returns the ImagePlus with the specified ID. 
		...
		the ID is zero. */
	public synchronized static ij.ImagePlus getImage(int imageID) {
		if (imageID>0)
	...
	public synchronized static ij.ImagePlus getImage(String title) {
		...
		for (int i=0; i<wList.length; i++) {
			ij.ImagePlus imp = getImage(wList[i]);
	...
	private static void addImageWindow(ImageWindow win) {
		ij.ImagePlus imp = win.getImagePlus();
		if (imp==null) return;
	...
	static void checkForDuplicateName(ij.ImagePlus imp) {
	...
	*/
	...
	public static String getUniqueName(ij.ImagePlus imp, String name) {
        String name2 = name;
        String extension = "";
	...
	public static ij.ImagePlus getTempCurrentImage() {
		return (ij.ImagePlus)tempImageTable.get(Thread.currentThread());
	}
    ...
    public synchronized static void repaintImageWindows() {
		...
		if (list==null) return;
		for (int i=0; i<list.length; i++) {
			ij.ImagePlus imp2 = getImage(list[i]);
			if (imp2!=null) {
