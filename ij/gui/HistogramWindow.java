...
public class HistogramWindow extends ImageWindow implements Measurements, ActionListener, 
	...
	private static final int SCALE = (int) Math.round(Prefs.getGuiScale()); // either 1, 2 or 3
	static final int HIST_WIDTH = SCALE * 256;
	static final int HIST_HEIGHT = SCALE * 128;
	static final int XMARGIN = 20;
	static final int YMARGIN = 10;
	static final int WIN_WIDTH = HIST_WIDTH + 28 * SCALE;
	static final int WIN_HEIGHT = HIST_HEIGHT + 98 * SCALE;
	static final int BAR_HEIGHT = SCALE * 12;
	...
	public void mouseMoved(int x, int y) {
		...
		if ((frame!=null)  && x>=frame.x && x<=(frame.x+frame.width)) {
			x = (x - frame.x);
			int index = (int)(x*((double)SCALE*histogram.length)/HIST_WIDTH/SCALE);
	...
	void drawText(ImageProcessor ip, int x, int y, boolean fixedRange) {
		ip.setFont(new Font("SansSerif",Font.PLAIN,12*SCALE));
		...
		int row1 = y+25* SCALE;
		if (showBins) row1 -= 8 * SCALE;
		int row2 = row1 + 15 * SCALE;
		int row3 = row2 + 15 * SCALE;
		int row4 = row3 + 15 * SCALE;
