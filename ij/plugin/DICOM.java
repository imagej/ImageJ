	...
	*/
	...
	public void run(String arg) {
		...
		if (fi!=null && fi.width>0 && fi.height>0 && fi.offset>0) {
			...
			// Avoid opening as float even if slope != 1.0 in case ignoreRescaleSlope or assumeFixedDicomScaling
			// were checked in the DICOM preferences.
			boolean openAsFloat = (dd.rescaleSlope!=1.0 && !(Prefs.ignoreRescaleSlope || Prefs.assumeFixedDicomScaling)) 
			...
			if (openAsFloat) {
				...
				if (dd.rescaleIntercept!=0.0 && (dd.rescaleSlope==1.0||Prefs.assumeFixedDicomScaling)) {
					...
					imp.getCalibration().setFunction(Calibration.STRAIGHT_LINE, coeff, "Gray Value");
					  ...
					  (dd.rescaleSlope==1.0||Prefs.assumeFixedDicomScaling||fi.fileType==FileInfo.GRAY8)) {
				double[] coeff = new double[2];
