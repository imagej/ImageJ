...
/**
This plugin saves stacks in AVI format, uncompressed 8-bit (gray or
indexed color) or 24-bit (RGB). 16-bit images are converted to 8-bit.
Zhe plugin is based on the FileAvi class written by William Gandler.
...
public class AVIWriter implements PlugInFilter {
    ...
    private int         zDim;           //number of frames (stack size)
    ...
    public void writeImage(ImagePlus imp) throws IOException{
        ...
        if (minLineLength%4 != 0)
            linePad = 4 - minLineLength%4; //lines written must be a multiple of 4 bytes
        ...
        writeInt(0); // biCompression - type of compression used, here BI_RGB=0
        writeInt(0); // biSizeImage, may be 0 for uncompressed data
        ...
        for (z = 0; z < zDim; z++) {
            ...
            writeInt(frameDataSize); // Write the data length
            if (bytesPerPixel==1)
                writeByteFrame(z+1);
            else
                writeRGBFrame(z+1);
        ...
        for (z = 0; z < zDim; z++) {
          // In the ckid field write the 4 character code to identify the chunk 00db or 00dc
           ...
           if (z == 0) {
             ...
             writeInt(frameDataSize); // Write the length of the relevant
